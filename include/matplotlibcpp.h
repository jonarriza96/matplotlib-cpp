...
PyObject *chart(int place) {
  PyObject *drawObject =
      PyObject_GetAttrString(detail::_interpreter::get().pymod, "subplot");
  PyObject *args = PyTuple_New(1);
  PyObject *kwargs = PyDict_New();
...
#ifndef WITHOUT_NUMPY
template <typename Numeric>
void surface3D(PyObject *ax, const std::vector<::std::vector<Numeric>> &x,
               const std::vector<::std::vector<Numeric>> &y,
               const std::vector<::std::vector<Numeric>> &z, std::string color,
               double linewidth) {
...
template <typename Numeric>
void axis_equal(const std::vector<Numeric> &x, const std::vector<Numeric> &y,
                const std::vector<Numeric> &z, const long fig_number = 0) {
  ...
  // get axis limit objects
  PyObject *xlim = PyObject_GetAttrString(axis, "set_xlim");
  PyObject *ylim = PyObject_GetAttrString(axis, "set_ylim");
  PyObject *zlim = PyObject_GetAttrString(axis, "set_zlim");
  ...
  double max_range =
      std::max({(maxX - minX), (maxY - minY), (maxZ - minZ)}) / 2.0;
  double mid_x = (x.back() + x.front()) * 0.5;
  double mid_y = (y.back() + y.front()) * 0.5;
  double mid_z = (z.back() + z.front()) * 0.5;

  // set limits
  PyObject *xlim_args = PyTuple_New(2);
  PyObject *ylim_args = PyTuple_New(2);
  PyObject *zlim_args = PyTuple_New(2);
  PyTuple_SetItem(xlim_args, 0, PyFloat_FromDouble(mid_x - 1.2 * max_range));
  PyTuple_SetItem(xlim_args, 1, PyFloat_FromDouble(mid_x + 1.2 * max_range));
  PyTuple_SetItem(ylim_args, 0, PyFloat_FromDouble(mid_y - 1.2 * max_range));
  PyTuple_SetItem(ylim_args, 1, PyFloat_FromDouble(mid_y + 1.2 * max_range));
  PyTuple_SetItem(zlim_args, 0, PyFloat_FromDouble(mid_z - 1.2 * max_range));
  PyTuple_SetItem(zlim_args, 1, PyFloat_FromDouble(mid_z + 1.2 * max_range));

  PyObject_CallObject(xlim, xlim_args);
  PyObject_CallObject(ylim, ylim_args);
  PyObject_CallObject(zlim, zlim_args);
}
